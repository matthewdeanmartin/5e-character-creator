...
init:  ## Initialize go.mod if it doesn't exist (needed on fresh clone)
	@if [ ! -f go.mod ]; then \
		$(GO) mod init character-creator && $(GO) mod tidy; \
		echo "go.mod created. Consider removing it from .gitignore."; \
	else \
		echo "go.mod already exists."; \
	fi

.PHONY: open
open:  ## Open the app in the default browser (macOS/Linux/WSL)
